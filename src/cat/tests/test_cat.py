...
files = [
    ...
    'test_files/4.file',
    'test_files/5.file',
    # 'test_files/big_file.file',
    'test_files/empty_lines.file',
    'test_files/empty.file',
    'test_files/eof.file',
    'test_files/eofs.file',
    'test_files/random.file',
    'test_files/v255_1.file',
    'test_files/v255_2.file',
    'test_files/v255.file',
    'test_files/nr.file',
    'test_files/nulls.file',
    'test_files/nulls2.file',
]
...
def simple_test():
    ...
    with open(list_test_file, 'r') as f:
        while(True):
            ...
            print(f'{bcolors.HEADER}======================================================================================={bcolors.ENDC}')
            argv2 = argv.replace("--squeeze-blank","-s").replace("--number-nonblank","-b").replace("--number","-n")
        ...
        for list_arg_m in combinations_with_replacement(flags, i):
            for list_arg in (set(list_arg_m), list_arg_m):
                ...
                for j in range(nums_of_test_cases_for_files):
                    ...
                    res_func = run_test_func(f'{s21_comm} {argv} > {s21_comm_file} 2> {s21_comm_error_file}', f'{comm} {argv} > {comm_file} 2> {comm_error_file}')
                    res_fsan = run_test_fsan(f'{s21_fcomm} {argv} > {s21_comm_file}')
                    res_valgrind = run_test_valgrind(f'{valgrind} {s21_vcomm} {argv} > {s21_comm_file}')
                    res_leaks = run_test_leaks(f'{leaks} {s21_vcomm} {argv} > {s21_comm_file}')
